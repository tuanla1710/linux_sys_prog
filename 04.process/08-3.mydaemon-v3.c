...
int fd;
const char pidfile[] = "/run/mydaemon/my-daemon.pid";
...
int main(void){
   ...
   /* the parent process has exited, which makes the rest of the code the child process */
   setsid(); 	/* create a new session to lose the controlling terminal */
   ...
   if ((fd = open(daemonfile, O_WRONLY | O_CREAT | O_APPEND, 0666)) == -1){
   ...
   while(1){
      ...
      time(&now);
      fprintf(stdout, "Daemon alive at %s", ctime(&now));
